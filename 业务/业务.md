...
2. 根据弹窗存在本地的数据
	...
	7. remark 备注
3. 查询上一次记录存在本地的数据
	1. serverResourceConfiguration 产品配置 取值 deployPlanContent.serverResourceConfiguration
	2. dataList 产品清单 取值 deployPlanContent.productDeployConfig
	3. 各个中间件信息 取值遍历 deployPlanContent.middlewareDeployConfig
	4. dataSourceList 数据源配置 取值deployPlanContent.datasourceDeployConfig
	5. serverList 服务配置 deployPlanContent.serverDeployConfig
...
第二步
1. 配置服务器，测试钩子，测试完毕后服务器互访 (钩子是根据架构和操作系统区分的，接口做的，取默认版本或者第一个)
第三步
1. 配置中间件，数据库，产品，然后存到
