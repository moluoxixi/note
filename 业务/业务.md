...
4. 35 处理包逻辑
	...
	2. 如果是中间件,redis集群
5. uploadProduct.js  产品下载逻辑: 上传产品数据库文件 +  (lvs/微服务/nginx)与他们的日志
6. RedisColony.js redis 下载逻辑: 上传 redis 日志
7. 高可用模式下, 安装完毕后把 nginx 刷到 lvs 里去,
8. 产品安装完毕, 调用updateVersionInstallTime 更新预计安装时间
