1. 每次查询上次记录都提示更新, 应该是版本列表返回的怀疑是顺序问题或者默认版本问题
2. 安装预估时间不对, 需要排查
## skywalking
...
4. 第四步
	...
	2. 点下一步
		...
		2. 计算安装时间, 调用 `/getInstallTime接口`, 强依赖productCode 字段, 根据`packageSize,predictInstallTime,downloadPackageTime` 计算安装时间
5. 第五步
	...
	6. 高可用模式下, 安装完毕后把 nginx 刷到 lvs 里去,
	7. 产品安装完毕, 调用updateVersionInstallTime 更新预计安装时间
6. 安装完成, 上传插件生成的日志 (安装的时候滚动的那个日志) 
### 安装
