...
- 用于捕获传统的 JavaScript 错误，如 throw new Error('Something went wrong');。

1. window.addEventListener('unhandledrejection', ...)：

- 用于捕获未处理的 promise 拒绝。当 promise 被拒绝且没有相应的 .catch() 处理程序时，会触发此事件。

1. Source Maps(源码映射)：

- 由于生产环境中的代码通常是压缩和混淆的，Sentry 支持上传 source maps，以便在报告中显示原始、未压缩的代码行和文件名。

1. Breadcrumbs(面包屑导航)：

- 用于收集应用程序运行时的事件。这些事件可以包括用户交互、网络请求、导航中的路由跳转以及系统信息等。

- 支持多种类型，如default（默认）、http（将面包屑呈现为HTTP请求）和error（将面包屑渲染为硬错误）等。

1. 自动会话跟踪：

- 跟踪各种交互过程，例如点击事件，自定义事件，页面停留时间等。
...
1. 单击 + Create Project 按钮


1. 根据您希望监控的代码为您的项目选择语言或框架在本例中为 JavaScript。

1. 给该项目一个 Name。

1. 在 Set your default alert settings 下，选择 i'll create my own alerts later。

1. 为该项目分配一个 Team。

![](images/WEBRESOURCEb3a2d406c764c821d18eeb281779bc19image.png)

1. 复制DSN key 并将其放在手边，因为我们会将密钥复制到源代码中。
...
1. 错误名称：通常与JavaScript中的错误类型相对应，如TypeError、ReferenceError、SyntaxError等。

1. 错误消息：具体的错误描述，例如“Cannot read property 'length' of undefined”。

1. 错误堆栈跟踪：显示错误发生时的函数调用堆栈，这有助于定位问题发生的具体位置。

1. Vue特定信息：

1. 组件信息：如果错误发生在Vue组件中，可能会包含组件的名称、类型以及相关的渲染函数或生命周期钩子信息。

1. 路由信息：如果配置了路由追踪（通过Sentry.vueRouterInstrumentation(router)），则可能包括发生错误时的路由路径和参数。
...
1. 屏幕截图：开启之后录制的过程中每一帧都会截图

1. 内存：开启之后会显示内存变化图

1. CPU：可以调节CPU的性能，一般用来模拟不好的设备

1. 网络：调节网络的速度

1. 硬件并发：设置CPU的核数，下图是谷歌官方文档的解释，是一个比较新的功能
...
1. 浏览器会将耗时的长任务在矩形的右上角进行红色的标记

1. 从上到下是调用栈（但是这个是类似事件捕获），从外到里的

1. 点击某一个任务可以定位到对应的源码上，方便定位问题
...
在控制台可以找到这个 javaScript性能分析器，英文 

![](images/WEBRESOURCE6803799c068d25899f9b5355bd7fd412截图.png)
...
1. 这个工具只是一个性能指标监视器，不能帮你定位是哪一行代码有问题。

1. 如果某个操作，导致CPU总是被拉满，就可以排查一下是不是有大量数据在计算，消耗CPU。

1. 如果某个操作，导致页面白屏、卡顿等，就关注一下【样式重新计算次数/秒】 这个指标的变化。

1. 如果某个操作，导致页面奔溃，就关注一下【js堆大小】指标的变化，如果这个指标呈现一个上升的趋势，则说明是有严重的内存泄露问题，如上面视频中的例子。
