...
2. renderer由brower创建，负责解析前端代码并渲染，同站应用可能会共享一个renderer进程
3. 每个tab页和每个进程都拥有自己的isostate，用于隔离环境,有独立的堆和栈，负责内存管理和垃圾回收等操作
4. context由isostate创建,用于实际执行代码，包含执行代码所需的必要变量
