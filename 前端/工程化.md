...
-->3.安装husky&lint-staged,在git commit时,通过husky的pre-commit钩子调用lint-staged校验暂存区的代码
	...
	//根目录创建.lintstagedrc.json 文件或在package.json中添加lint-staged配置项 添加以下代码
		...
		"*.{scss,less,js,jsx,ts,tsx,vue}": [  
		"prettier --write  \"src/**/*.{js,tsx,css,less,scss,vue,html}\"",  
		"stylelint --cache --fix \"**/*.{less,postcss,css,scss,vue}\" --cache --cache-location node_modules/.cache/stylelint/"  ],  
...
-->4.安装commitlint校验提交信息,也就是git commit -m "提交信息" 中双引号内容
	pnpm i -D commitlint @commitlint/config-conventional
	//在.commitlintrc.js中添加
	module.exports = {
	    "extends": [
