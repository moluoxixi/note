...
| **API 名称**                                | **常用参数**                                                                                                                                    | **功能描述** |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| `uni.getLocation({type, isHighAccuracy})` | `type`（坐标类型, wgs84 返回 gps 坐标，gcj02 返回可用于 wx. openLocation 的坐标）、<br>`isHighAccuracy`（是否高精度）<br>`highAccuracyExpireTime` 高精度超时时间，要求 3000ms 以上 | 获取用户地理位置 |
...
以下是关于小程序 **分包配置** 的详细说明，重新整理并解释每个配置的作用，使用 Markdown 语法编写：
...
打开微信开发者工具。
在工具栏中点击详情 -> 基本信息。
查看当前代码包大小，确保主包和分包的大小符合限制：
主包大小不超过 2MB。
所有分包大小不超过 20MB。
单个分包大小不超过 2MB。
点击详情 -> 本地设置，勾选上传时压缩代码，进一步优化包体积。