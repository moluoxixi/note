...
![[dataLoom/Untitled.loom]]

| 配置项                        | 描述                                 | 类型      | 常见配置示例                                                                                                                                                                                                    |
| -------------------------- | ---------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| plugins                    | 使用插件                               | Array   | plugins: [tooltip],                                                                                                                                                                                       |
| layout                     | 布局                                 | Object  | layout:layoutConfig,                                                                                                                                                                                      |
| node/edge/comboStateStyles | 设置节点/边/combo的不同状态样式                | Object  | node/edge/comboStateStyles: {<br>  active: {}, //高亮状态<br>  nactive:{}, //非高亮<br>  hover:{}, //悬浮<br>  selected:{} //选中<br>},                                                                              |
| defaultEdge/Combo/Node     | 所有边(即连线)/Combo/Node的默认样式和行为        | Object  | defaultEdge/Combo/Node:{<br>  ...elementConfig,<br>  labelCfg<br>},                                                                                                                                       |
| modes                      | 使用哪些模式,每个模式应用哪些行为                  | Object  | modes: {<br>  //'drag-canvas/combo/node', //画布/combo/节点可拖拽<br>  //'activate-relations', //高亮相同节点<br>  //'click-select' //点击选中节点<br>  //'create-edge' //创建边<br>  default: ['drag-node'], //节点可拖拽<br><br>}, |
| enabledStack               | 启用栈操作                              | Boolean | enabledStack:true                                                                                                                                                                                         |
| fitView                    | 自动缩放以适应其容器的大小(填充整个可视区域，而不会超出容器的边界) | Boolean | fitView: true,                                                                                                                                                                                            |
| fitCenter                  | 自动适应容器大小，并居中显示                     | Boolean | fitCenter: true,                                                                                                                                                                                          |
...
| 配置项 | 描述 | 类型 | 常见配置示例 | 
| -- | -- | -- | -- |
| nodes | 节点配置项 | Array | nodes: [ | 
| edges | 边配置项 | Array | edges: [ | 
| combos | combo配置项, | Array | combos: [ | 
|   |   |   |   | 
...
| 配置项 | 描述 | 类型 | 示例 | 
| -- | -- | -- | -- |
| parentId( | 父级ID,只能是combo的Id | String | combos: [ | 
| offset( | 折现的偏移量,就是从多长开始折 | Number |   | 
| source/targetAnchor( | 边的起始/终止起点连接的连接桩的下标 | Number | targetAnchor:0 | 
| source/target( | 连线的起点/终点对应的(节点/combo的id/{x,y}) | String||Number |   | 
| anchorPoints | 设置元素的锚点 | Array | anchorPoints:[ | 
| x/y( | 节点的坐标 | Number |   | 
| r( | /圆的半径 | Number |   | 
| id | 标识 ID， | String |   | 
| opacity | 透明度 | Number | opacity:0.3 | 
| size | edge中对应边的粗细,size:1, | Array||Number |   | 
| style | 元素  | Object | 见styleConfig | 
| type | 元素的类型，不传则使用默认值， | String |   | 
|   | 元素的文本标签,有该字段时默认会渲染 label  | String |   | 
|   | 元素label标签的配置项 | Object |   | 
...
| 名称 | 备注 | 类型 | 
| -- | -- | -- |
| position | 自定义节点不支持该属性 | String | 
| refX/refY(combo与边特有) | label在x,y方向的偏移量 | Number | 
| autoRotate(边特有) | 标签文字是否跟随边旋转，默认 | Boolean | 
| offset | 文本的偏移， | Number | 
| style | 标签的样式属性，具体配置项参见统一整理在 | Object | 
...
| 名称 | 类型 | 默认值 | 描述 | 
| -- | -- | -- | -- |
| type | String | dendrogram | 布局类型，支持  | 
| excludeInvisibles | Boolean | false | v4.8.8 起支持。 | 
| direction | String | LR | 布局方向，有   | 
...
| graph.addItem(type, model, stack) | 新增元素 | type:元素的类型 | 
| updateItem | 更新元素 | item:元素的id/实例 | 
| graph.removeItem(item, stack) | 移除元素 | item:元素的id/实例 | 
| graph.createCombo(combo, elements, stack) | 创建combo | combo: | 
| graph.collapseExpandCombo(combo) | 展开或收缩指定的 Combo | combo: | 
| graph.downloadFullImage(name, type, imageConfig) | 将画布上的元素导出为图片 | name, | 
| graph.toFullDataURL(callback, type, imageConfig) | 将画布上元素生成为图片的 URL。 | callback, | 
...
| node.getNeighbors(type) | type有 | 
| node.getEdges/getIn/OutEdges() | `getEdges`获取当前节点有关联的所有边( | 
