[https://g6.antv.antgroup.com/api/graph](https://g6.antv.antgroup.com/api/graph)
...
vue2中动态引入图片可使用require函数

vue3中不支持require,需要先引入图片
...
| node/edge/comboStateStyles | 设置节点/边/combo的不同状态样式 | Object | node/edge/comboStateStyles: { | 
| defaultEdge/Combo/Node | 所有边(即连线)/Combo/Node的默认样式和行为 | Object | defaultEdge/Combo/Node:{ | 
| modes | 使用哪些模式,每个模式应用哪些行为 | Object | modes: { | 
