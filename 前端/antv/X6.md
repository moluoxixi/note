...
|   | boolean||Object | 背景网格的配置 | grid:true | 
|   | Object | 配置背景 |   | 
|   | boolean||Object | 画布是否可拖拽平移 | panning:true | 
|   | boolean||Object | 画布是否可缩放(ctrl加滚轮) |   | 
|   | Object | 画布的最大最小缩放级别 |   | 
|  | Object | 交互 | connecting:connectingCfg | 
|  | Object | 高亮器,支持以下配置项 | highlighting: { | 
...
| 配置项 | 类型 | 描述 | 示例 | 
| -- | -- | -- | -- |
|   | boolean||string | 是否开启自动吸附, | snap: { | 
| allowBlank | boolean|| | 是否允许指向画板 |   | 
| highlight | boolean | 拖动边时，是否高亮所有可用连接桩或节点 |   | 
| router | string/object | 边的路径点  | router:{ | 
| createEdge | ( | 通过该方法自定义新建边的样式 |   | 
| connector | string/object | 连接器 |   | 
| validateEdge | ( | 当停止拖动边时 |   | 
| validateMagnet | ( | 校验哪些元素可作为磁力点(magnet), |   | 
| validateConnection | ( | 校验哪些元素可被连接 |   | 
|  | Object/String |  |   | 
|  | Object/String |  |   | 
...
| zIndex | Number | 层级,一般用来设置边,防止边覆盖节点 |   | 
...
| x/y | number | 节点的定位 | x:120,y:10 | 
| label | string | 节点的文本 |   | 
| component |   | 使用组件 | component:MyComponent | 
|   |   |   |   | 
| angle | number | 节点的旋转角度。 |   | 
| ports | object | 连接桩。 | 见节点/连接桩 | 
|   |   |   |   | 
| source/target | TerminalData | 起点/终点 连接的节点id以及连接桩/锚点信息。 | target: { | 
| vertices | Point.PointLike[] | 路径点。 | vertices: [ | 
| router | RouterData | 路由。 | router:{ | 
| connector | ConnectorData | 连接器。如何连接路径点 | connector:{ | 
|   |   |   |   | 
...
| textAnchor | String | 文本基于其锚点的水平对齐方式,值:start,middle(默认),end |   | 
| textVerticalAnchor | String | 文本基于其锚点的垂直对齐方式,值:top,middle(默认),bottom |   | 
| ref | String | 指定 | ref | 
|   | String | 向x/y轴平移的百分比或偏移量 |  refX | 
| refX2/Y2 | String | 通过 | refX | 
| refDx/Dy | String | 设置元素坐标为相对于 |   | 
| refRx/y | String | 椭圆半径 | refRy | 
| refCx/y | String | 椭圆圆心 | refCx | 
| refWidth/Height | String | 宽高为 | refWidth | 
| refWidth2/Height2 | String | 同 | refWidth | 
| source/targetMarker( | String|Object | 边的 |   | 
...
| 工具类型 | 描述 | 
| -- | -- |
|   | 渲染一个包围元素的矩形。注意，该工具仅仅渲染一个矩形，不带任何交互。 | 
|   | 在指定位置处渲染一个按钮，支持自定义按钮的点击交互。 | 
|   | 在指定的位置处，渲染一个删除按钮，点击时删除对应的元素 | 
| (节点特有) | 提供文本编辑功能。 | 
| (边特有) | 提供文本编辑功能。 | 
| (边特有) | 路径点工具，在路径点位置渲染一个小圆点，拖动小圆点修改路径点位置，双击小圆点删除路径点，在边上单击添加路径点。 | 
| (边特有)  | 线段工具。在边的每条线段的中心渲染一个工具条，可以拖动工具条调整线段两端的路径点的位置。 | 
| (边特有)  | 在边的起点或终点渲染一个图形(默认是箭头)，拖动该图形来修改边的起点或终点。 | 
...
| graph | resize画布大小 | 
| graph. | 将画布中元素居中展示 | 
| graph | 将画布中的元素自适应到刚好放下 | 
| graph.getNeighbors(cell, | 获取节点相邻的节点,incoming是输入节点,outgoing是输出节点 | 
| graph.getPredecessors/Successors(cell) | 获取节点的所有前序/后续节点, | 
| graph.is | 判断cell2是不是cell的相邻/前序/后续节点 | 
| graph.getConnectedEdges(node,  | 获取节点所连接的边,incoming是输入边,outgoing是输出边 | 
