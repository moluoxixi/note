...
4. 新增 Fragments（片段）：Vue 3 中**添加了 Fragments**，也称为空内容标签，允许组件返回多个根元素，不需要每个 template 必须用单元素包裹了。
5. 移除$set
6. 移除 Vue.extend, 直接使用 createApp 代替,
7. 移除.native, 现在绑在组件上的属性不会默认被根组件继承了
8. 所有依赖改为 esModule 形式, 方便树摇, 例如 vue 实例现在需要通过 createApp 创建
9. 提供 composition api
