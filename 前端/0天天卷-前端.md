...
7. 移除.sync
8. v-model 改为与.sync: modelValue 等同用法, 即: modelValue + @update:modelValue
9. 所有依赖改为 esModule 形式, 方便树摇, 例如 vue 实例现在需要通过 createApp 创建
10. 提供 composition api
11. 新增 Fragments（片段）：Vue 3 中**添加了 Fragments**，也称为空内容标签，允许组件返回多个根元素，不需要每个 template 必须用单元素包裹了。
12. (这个有待考察)获取路由实例与当前路由实例：vue 2 通过 this 获取 router 实例，vue 3 通过使用 getCurrentInstance/ userRoute 和 userRouter 方法获取当前组件实例
13. (这个有待考察)Vuex 换成 pinia, 从 new store 变成 createStore
