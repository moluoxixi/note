...
## 2/3 使用区别
1. setup 替代之前的 beforecreate 和 created
2. Vuex 换成 pinia, 从 new store 变成 createStore
4. 新增 Fragments（片段）：Vue 3 中**添加了 Fragments**，也称为空内容标签，允许组件返回多个根元素，避免了在模板中必须包裹在一个父级元素内的限制。
5. (zh)获取路由实例与当前路由实例：vue 2 通过 this 获取 router 实例，vue 3 通过使用 getCurrentInstance/ userRoute 和 userRouter 方法获取当前组件实例
