...
项目如何选型
跟着react小册学完
webpack实操,如何 配置静态资源路径,分包,gzip压缩
```
...
但是如果组件内不引入直接使用, eslint 会报错, 此时需要将 ElMessage 声明为全局变量
```js
...
见[代码实现](note://WEB4dc6b17eee78965cf899c61fb4f71829)
...
但是当jsx语法放到外部js中引入会报错,因为jsx需要关联组件上下文,否则无法解析(编译的时候h函数在vue组件内,如果放到外部js,外部文件h函数不存在,会直接报错)
...
1. Setup 替代之前的 beforecreate 和 created, **升级需将 created 换成 beforeMount**
2. 移除 `$on` `$off` 和 `$once`, 可使用第三方包 `mitt` 代替,
3. 移除 `$children`
4. 移除过滤器 `filter`
5. 移除$set
6. 移除 Vue. Component, 导致无法直接通过 this 传递属性, 使用 `app.config.globalProperties` 代替
7. 移除 Vue. Extend, 直接使用 createApp 代替,
8. 移除. Native, 现在绑在组件上的属性不会默认被根组件继承了
9. 移除. Sync, v-model 改为与 `.sync` 等同用法, 默认 v-model 等同于 `v-model:modelValue`, 即 vue 2 的 `:modelValue.sync` 也就是 `:modelValue` + `@update:modelValue`
10. 移除 `$listeners` 合并到 `$attrs` 中, `$attrs` 现在包含了所有_传递给组件的 attribute，包括 `class` 和 `style`。
11. 移除 `v-on` 的 `.native` 修饰符除。新增 Fragments（片段）：Vue 3 中**添加了 Fragments**，也称为空内容标签，允许组件返回多个根元素，不需要每个 template 必须用单元素包裹了。
12. 移除 `v-on` 的数字 (即键码) 修饰符
13. 移除 `$scopedSlots`, 统一使用 `$slot`,2.x 语法 `this. $scopedSlots.header` ,3.x 语法 `this.$ slots.header()`
14. 新增emits 选项, 可以声明组件可触发的事件, `emits: ['accepted']`
15. 修改, h 函数现在是全局导入, 不再传递给 render 函数
16. 修改 ,对于 `v-if` / `v-else` / `v-else-if` 的各分支项 `key` 将不再是必须的，因为现在 Vue 会自动生成唯一的 `key`。如果你手动提供 `key`，那么每个分支必须使用唯一的 `key`。你将不再能通过故意使用相同的 `key` 来强制重用分支。
17. 修改 ,2.x 版本时, `template` 不能设置 key, key 设置在 `template` 的子节点上, 3.x 时 `<template v-for>` 的 `key` 应该设置在 `<template>` 标签上 (而不是设置在它的子节点上)。
18. 修改 ,2.x 版本中在一个元素上同时使用 `v-if` 和 `v-for` 时，`v-for` 会优先作用。3.x 版本中 `v-if` 总是优先于 `v-for` 生效。**建议避免在同一元素上同时使用两者。**
19. 2.x 中，如果一个元素同时定义了 `v-bind="object"` 和一个相同的独立 attribute，那么这个独立 attribute 总是会覆盖 `object` 中的绑定。3. X 版本根据绑定的声明顺序将决定它们如何被合并, 即 `v-bind="object"` 在 bind 后则 `v-bind="object"` 优先级高, 否则相反。
20. 所有依赖改为 esModule 形式, 方便树摇, 例如 vue 实例现在需要通过 createApp 创建
21. 提供 composition api
22. (这个有待考察) 获取路由实例与当前路由实例：vue 2 通过 this 获取 router 实例，vue 3 通过使用 getCurrentInstance/ userRoute 和 userRouter 方法获取当前组件实例
23. (这个有待考察) Vuex 换成 pinia, 从 new store 变成 createStore
## 2/3 原理上的区别

1. 源码用ts重写
2. 响应性系统优化：Vue 2使用了Object.defineProperty来实现响应式系统，Vue 3在reactive和ref接受复杂类型时使用了Proxy来实现,仅在ref接受基本类型时使用defineProperty代理。
3. defineProperty在对象新增和删除属性时,数组新增,删除,修改属性以及通过length改变长度时不具有响应式
4. proxy支持更多数据类型的劫持,除Object和Array外,还支持Map,Set,WeakSet,WeakMap
5. proxy劫持对象本身,defineProperty劫持对象中所有的属性
6. proxy劫持数组本身,vue2只能通过数组方法实现对数组的监控
7. 性能优化：
	- diff优化:
		- Vue 3在将模板转换为AST树时将动态改变节点结构的部分(例如vif,vfor)放在一个数组中,节点改变时从数组中取值,避免每次动态改变节点重新生成虚拟DOM递归比较
		- AST树生成后遍历AST树寻找静态节点并打上静态标识,在生成高性能渲染函数时将静态节点提取到渲染函数之外，避免每次渲染时重新创建
	- 缩小包大小：Vue 3在减少包大小方面进行了大量优化，通过利用Tree shaking和静态提升（Static Hoisting）等技术，消除了一些和应用无关的代码，提供了更小的打包体积。
		- 树摇：依赖es module的import和export,通过分析引用关系摇掉冗余代码
		- 静态提升：就是在模板编译阶段在ast解析使标记静态节点,生成渲染函数时将静态节点提升到render函数外面
			- 模板解析的parse阶段生成ast
			- optimize：遍历 ast ，标记静态节点，知道哪部分不会变化，于是在页面需要更新时，通过 diff 减少去对比这部分DOM，提升性能
			- generate 把前两步生成完善的 ast，组成 render 字符串，并把静态节点提升到render外面，然后将 render 字符串通过 new Function 的方式转换成渲染函数
...
```
//总结如下,计算属性依赖更新,触发get,普通响应式更新,触发渲染函数
​
Vue响应式的核心分为三大模块:Obsever,Compile,Watcher
​
利用dep连接Obsever和Watcher,
​
初始化阶段Obsever将数据 代理+劫持,3用proxy,但是Proxy只能代理一层操作,内部会判断如果是对象/数组的话,会进行深层代理处理
编译阶段，让dep连上当前的Watcher----->就是编译的时候创建一个watcher,碰到一个动态绑定存一个dep到watcher中
然后更新的时候对比dep变化
变化了就把dirty设置为true更新
dep存Watcher的deps数组 watcherd存dep的subs
​
​
compile和watcher都是作用于视图的,二者没有必然的联系,仅仅compile生成的渲染函数要传入watcher用于通知视图更新
​
Compile 扛派噢
用来解析和编译vue的模板与指令,生成渲染函数
​
Watcher
收集依赖,通知视图更新
```
...
-->垃圾回收机制
    ...
    清除时,将所有可达对象复制到空闲区排序,不可达对象清除,使用区与空闲区交换,
    老生代仅清除不可达对象并排序
​
-->执行上下文
...
```
-->继承
原型链继承:父类构造函数的实例赋值给子类原型对象,并重新将构造器指向子类
借用构造函数继承:子类构造函数内 通过强制绑定使父类函数this指向子类构造函数实例也就是子类的this
组合继承:以上两种继承的结合
es6 extends继承
​
-->深浅拷贝
...
2. 定义一个函数用于接收后端返回的数据, 比如 json 串, 例如 `const abc=(data) { Return data}`
3. Script 标签指向服务器的 get接口, 用于指定调用的函数名, 例如 `http://localhost:3000/testAJAX?callback=abc`
4. 该接口返回一个字符串用于调用函数并接收 json 串, 例如 ` callback(json 串) ` 

5. 将要响应的数据以参数的形式传给该函数, 然后以参数的形式响应回 script 标签,

6. Script 标签自动执行响应回来的 js 脚本代码, 并该 script 标签内书写这个方法, 得到传入的数据
...
- 请求头中仅包含安全的字段, 常见的安全字段包括：
	...
	- Content-Type：对于 POST 请求，其值必须是 `application/x-www-form-urlencoded`、`multipart/form-data` 或 `text/plain` 中的一个。这些类型主要用于表单数据的提交，对服务器的影响较小。
