...
```
1. 控件联动             预计需要一个月
2. 增加布局容器      底层重构，大概需要半个月
3. 自定义属性支持  不知道作用是什么，需要一周

4. 另存为模板功能   需要设计效果，根据效果再评估，2-3天
5. 结果转换规则配置（需内置一个转中文函数），支持预览结果   1天
6. 提供单独界面，支持url传入id变成页面，页面仅展示表单，通过调用特定api与5.0通信（需要询问志哥与强哥）    2-3天
...
```
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
```

```
参考该项目在根目录建一个ConfigForm实现一个配置化表单组件，要求如下：  
1. 要求用vue3+ts，  无须package.json，它是一个组件文件夹
2. 假设element-plus存在，检查line时无须检查element-plus
3. 所有element-plus的组件都应该从element-plus中导入
4. 支持通过配置的JSON传入要用的组件，  数据结构参考该项目

5. ConfigForm的目录结构应该是  
	ConfigForm  
	    src  
	        _types //类型文件  
	            props //defineProps的类型  
	            emits //emits的类型  
	            index.ts  
	        Example.vue //示例文件  
	        index.vue  
...
core
	index.ts //代替draft.ts
	media 文件夹
		index.ts //导出MediaFactory，MediaEffect，Media
		media.ts
		template //模板目录，代替template.ts
			template.ts //代替template目录下draft.ts
			... //其他文件，按音频，视频，动画，转场等，按功能拆分为多个文件
			index.ts //统一导出
		mediaEffect.ts
		mediaFactory //文件夹
			index.ts //导出MediaFactory
			mediaAudio.ts
			mediaImage.ts
			mediaText.ts
			mediaVideo.ts
