...
class editorTableOperation extends editorElementOperation{
	...
	//表格操作
		...
		// 获取表格指定容器内的输入域
			getInputFields(element,excludeReadonly=false,excludeHiddenElement=false){
				let Element=element;
				if(this.getType(element)==='String'){
					if(element==='table')Element=this.editor.CurrentTable();
					if(element==='row')Element=this.editor.CurrentTableRow();
					if(element==='column')Element=this.editor.CurrentTableColumn();
					if(element==='cell')Element=this.editor.CurrentTableCell();
				}
				return this.editor.GetAllInputFields(excludeReadonly, excludeHiddenElement, Element, true)
			}

		// 设置列/行对象的所有单元格的属性  
		  	setAllCellsProperties = (element, callback, excludesCallback) => {  
				const editor = props.editor  
				const Element = this.getElementOrProperty(element)
				Element.Cells.forEach((cell) => {  
					//什么条件下不需要设置属性
					if (this.getType(excludesCallback) === 'Function') {  
						if (excludesCallback(cell)) return  
					}  
					...
					//仅设置属性
					 else if (this.getType(callback) === 'Object') {  
						this.setElementProperties(cell, data)  
					}  
				})
			}
...
class editorDataElementOperation extends editorElementOperation{
	...
	setDefaultValue(element,value){
		const Element=this.getElementOrProperty(element)
		this.setElementOrProperty(Element,{
			...Element,
			Text: value,
			InnerValue: value
		})
