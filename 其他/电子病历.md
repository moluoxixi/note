...
| SetChildElements (element/element.ID, options, position,isRefresh,isAddTrack) | options: 插入元素列表<br>position:插入位置，<br>    afterBegin 开始位置<br>    beforeEnd 开始位置<br>    beforebegin 最后位置<br>isRefresh: 是否刷新<br>isAddTrack: 是否带痕迹<br><br>var options = [<br>    {<br>        'image':{//图片<br>            id: "123",  //图片id<br>            //src: redpicbase64data, //图片路径<br>            src: "http://localhost:5214/pingcode/webdemo.png", //图片路径<br>            width: "100",  //图片宽度<br>            height: "100",  //图片高度<br>            savecontentinfile: false<br>        }<br>        'text':"文本1\r\n文本2"<br>    }<br>]; |
