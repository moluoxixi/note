...
| -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|                                                                            |                                                                                                                                                                                                                                                                                                                             |
| getElementOrProperty (element/element. ID)                                 | 获取对象身上的属性                                                                                                                                                                                                                                                                                                                   |
| SetElementProperties (element/element.ID, data)                            | 设置对象身上的属性                                                                                                                                                                                                                                                                                                                   |
...
|                                                                            |                                                                                                                                                                                                                                                                                                                             |
| GetAllInputFields(excludeReadonly,excludeHiddenElement,specifyRootElement) | excludeReadonly 是否排除只读元素<br>excludeHiddenElement 是否排除隐藏元素<br>specifyRootElement 获取指定容器对象下的输入域集合列表。<br><br>比如需要获取一个输入域里面所有子输入域元素，传父元素的编号即可<br><br>比如需要获取一个表格所有输入域，传表格编号即可<br><br>比如需要获取一个病程中所有输入域，传病程编号即可<br><br>比如需要获取页眉中所有输入域传参为 header<br><br>比如需要获取正文中所有输入域传参为 body<br><br>比如需要获取页脚中所有输入域传参为 footer<br><br>不传参数默认获取全文输入域 |
|                                                                            |                                                                                                                                                                                                                                                                                                                             |
## 操作 json
```js
//操作JSON
...
### 封装实用方法
```js
class editorTableOperation extends editorElementOperation{
	constructor(editor){
		super(editor)
		this.editor=editor;
	}
	...
	//表格操作
		...
		// 获取表格的所有列  
			getTableColumns (element){  
			  const editor = this.editor  
			  const Table = this.getElementOrProperty(element)  
		...
		// 设置列/行对象的所有单元格的属性  
		  	setAllCellsProperties (element, data, callback){  
		  	  const editor = this.editor  
		  	  const Element = this.getElementOrProperty(element)  
		  	  Element.Cells.forEach((cell) => {  
		  	    setElementProperties(cell, data)  
		  	    callback?.(cell)  
		  	  })  
		  	}
	  	
		// 获取列中的第一个数据元  
		  	getColumnDataElement (element) {  
		  	  const editor = this.editor  
		  	  const Column = this.getElementOrProperty(element)  
		  	  let dataElement = null  
		  	  for (const cell of Column.Cells) {  
		  	    if (dataElement) break  
		  	    const Cell = this.getElementOrProperty(cell)  
		  	    if (Cell) {  
		  	      for (const item of Cell.Elements) {  
		  	        const Element = this.getElementOrProperty(item)  
		  	        if (Element.TypeName == 'XTextInputFieldElement') {  
		  	          dataElement = Element  
		  	          break  
...
| specifyRootElement   | 非必填  | String | 获取指定容器对象下的输入域集合列表。<br><br>比如需要获取一个输入域里面所有子输入域元素，传父元素的编号即可<br><br>比如需要获取一个表格所有输入域，传表格编号即可<br><br>比如需要获取一个病程中所有输入域，传病程编号即可<br><br>比如需要获取页眉中所有输入域传参为 header<br><br>比如需要获取正文中所有输入域传参为 body<br><br>比如需要获取页脚中所有输入域传参为 footer<br><br>不传参数默认获取全文输入域 |
## 输入域相关 api
...
class editorDataElementOperation extends editorElementOperation{
	...
	setDefaultValue(element,value){
		const Element=this.editor.GetElementProperties(element)
		this.editor.SetElementProperties(Element,{
			...Element,
			Text: value,
			InnerValue: value
		})
...
# 业务代码
```
console.log('abc', getEditor().editor.GetAllInputFields(false, false, 'header', true))
```

```
// 设置为签名后,签名之后与签名列相同分组列的当前行无法编辑  
function handle() {  
  const editor = props.editor;  
  const curentCell=editor.CurrentTableCell();  
  const CurrentCell=editor.GetElementProperties(curentCell);  
  const groupName=CurrentCell.Attributes.groupName;  
  const row = editor.CurrentTableRow()  
  const Row = editor.GetElementProperties(row)  
  const alikeCells = []  
  Row.Cells.forEach((cell) => {  
    const Cell = editor.GetElementProperties(cell)  
    if (Cell.Attributes?.groupName == groupName) {  
      alikeCells.push(Cell)  
    }  
  })  
  
  if (alikeCells.find((Cell) => Cell.Attributes.isSign == '1')) {  
    alikeCells.forEach((Cell) => {  
      setElementProperties(Cell, {  
...
```
<el-button @click.stop="handleAddRow(false)">新增行</el-button>  
<el-button @click.stop="handleAddRow(true)">追加行</el-button>  
<el-button @click="handleDeleteRow">删除行</el-button>


// TODO: 追加行时,仅追加空白行  
// TODO: 新增行时日期和时间取当前日期时间(分两行)  
// TODO: 每一列需绑定指定数据元,需要一个自定义属性存它,并在新增时取出来  
  
const handleAddRow = (isCopy) => {  
  const editor = getEditor().editor;  
  // const table = editor.GetElementProperties(editor.CurrentTable())  
  if (isCopy) {  
    const currentRow = editor.CurrentTableRow()  
    const currentRowInputFields = editor.GetAllInputFields(false, false, currentRow, true)  
    editor.DCExecuteCommand('Table_InsertRowDown', false, null)  
    const insertRow = editor.CurrentTableRow()  
    const insertRowInputFields = editor.GetAllInputFields(false, false, insertRow, true)  
  
    const decodes = [  
      'DE02.01.005.00', // 日期  
      'DE06.00.062.00' // 时间  
    ]  
    currentRowInputFields.forEach((e, i) => {  
      decodes.forEach((decode) => {  
        if (e.Attributes.deCode === decode) {  
          editor.SetElementProperties(insertRowInputFields[i], {  
            ...e,  
            Visible: false,  
            ContentReadonly: 'True'  
          })  
    ...
    // editor.DCExecuteCommand('Table_InsertRowDown', false, {  
    //   tableid: table.ID, //表格id  
    //   rowindex: table.Rows.length, //表格的索引0开始计算  
    //   rowcount: 1 //插入的行数  
    // })  
  }  
}  
...
const handleDeleteRow = (e) => {  
  e.stopPropagation()  
  getEditor().editor.DCExecuteCommand('Table_DeleteRow', false, null)  
}
```