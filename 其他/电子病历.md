...
//元素操作
	...
	// 获取元素中的第一个数据元  
		const getDataElement = (element) => {  
		  const editor = getEditor().editor  
		  const El = editor.GetElementProperties(element)  
		  let dataElement = null  
		  for (const item in El.Elements) {  
		    const Element = editor.GetElementProperties(item)  
...
//表格操作
	...
	// 获取表格的所有列  
		const getTableColumns = (element) => {  
		  const editor = getEditor().editor  
		  const Table = editor.GetElementProperties(element)  
	...
	// 设置列/行对象的所有单元格的属性  
	  	const setAllCellsProperties = (element, data, callback) => {  
	  	  const editor = getEditor().editor  
	  	  const Element = editor.GetElementProperties(element)  
    ...
    // 获取列中的第一个数据元  
	  	const getColumnDataElement = (element) => {  
	  	  const editor = getEditor().editor  
	  	  const Column = editor.GetElementProperties(element)  
	  	  let dataElement = null  
...
| api                                             | 说明           |
| ----------------------------------------------- | ------------ |
| GetElementProperties (element/element. ID)      | 获取对象身上的属性    |
