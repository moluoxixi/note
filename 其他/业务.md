...
1：被红冲之后的原始记录
2：冲正记录（也就是红冲的那一条正记录）
3：取消冲正记录（把红冲冲回来的那一条记录（负记录））
4：被取消冲正记录（冲正记录被取消后的状态）

vxe-grid的column的slots一共有如下插槽：{
default：自定义显示内容模板，
header：自定义表头内容的模板，
footer：自定义表尾内容的模板，
title：只对 type=checkbox,radio 有效，自定义标题模板，
checkbox：只对 type=checkbox 有效，自定义复选框模板，
radio：对 type=radio 有效，自定义单选框模板，
content：只对 type=expand 有效，自定义展开后的内容模板，
filter：只对 filter-render 启用时有效，自定义筛选模板，
edit：只对 edit-render 启用时有效，自定义可编辑组件模板，
valid：只对 edit-rules 与 edit-render 启用时有效，自定义校验提示模板，
}
slotsDiff为没有使用的插槽名称，给组件添加默认slots，使用defaultSlots存储，可被column传递的slots覆盖，
