...
```
渲染器放在DraggableTable/renderers目录中，参考  
https://vxetable.cn/#/global/renderer/table/filter/myFilterContent ，注意使用tsx语法  
为该组件实现一个filter渲染器，由props.filterable开启，开启后，如果column没有filters和filterRender和slots.filter，则默认使用该渲染器，通过props.filterLayout控制支持什么搜索，如果filterLayout为[input,checkbox,select],则从上到下同时开启input,checkbox,select搜索
